...
export default function GiftLandingPage() {
  ...
  const signOut = useGiftStore((s) => s.signOut)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#fff0f5] to-[#fce4ec] flex flex-col items-center justify-center p-6">
      ...
      >
        ...
        <p className="text-[#333] text-lg mb-2">조각을 모아 선물을 완성해요</p>
          ...
          <>
            <div className="bg-white rounded-2xl p-4 shadow-sm mb-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="text-xs text-[#555]">로그인됨</p>
                  <p className="font-bold text-[#333]">{currentUser.nickname}</p>
                </div>
              </div>
              <button
                onClick={signOut}
                className="text-xs text-[#999] border border-gray-200 px-3 py-1.5 rounded-xl hover:bg-gray-50"
              >
                로그아웃
              </button>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <Link href="/gift/join">
                <motion.button
                  whileTap={{ scale: 0.97 }}
                  className="w-full bg-white text-[#d63384] font-bold py-4 rounded-2xl text-lg border-2 border-[#d63384]"
                >
                  🔗 링크로 참여하기
                </motion.button>
              </Link>
              <Link href="/gift/history">
                <motion.button
                  whileTap={{ scale: 0.97 }}
                  className="w-full bg-white text-[#333] font-semibold py-3 rounded-2xl text-sm border border-gray-200"
                >
                  📜 선물 히스토리
                </motion.button>
              </Link>
